...
func main() {
	...
	//Connect to a broker
	client := client.GetSimpleMessageBrokerClient(*serverAddress, client.WithCustomListenPort(10000))
	...
	err := client.Subscribe(topic, func(data string, topic string) {
		...
		log.Default().Printf("Rcv %d\n", seqnum)
		receivede2e[seqnum] = int(currTime) - timesent
		if currTime > endTime {
			endTime = currTime
		}
	...
	for i := 0; i < (*pubMessages); i++ {
		...
		err = client.Publish(fmt.Sprintf("%d,%d", timeNow, i), topic)
		if err != nil {
